...
frappe.ui.form.on('MEAL Plan Tool', {
	setup: function(frm) {
	...
	project: function(frm) {
		...
		if(frm.doc.project) {
			frappe.call({
				method: "program_management.meal.api_meal_plan.get_indicators",
				args: {
	...
	render_table: function(frm) {
		$(frm.fields_dict.meal_plan_html.wrapper).empty();
			frm.events.get_marks(frm);
	},

	get_marks: function(frm) {

		var meal_plan_table = $(frappe.render_template('meal_plan_tool', {
			frm: frm,
			indicators: frm.doc.indicators
		}));
		meal_plan_table.appendTo(frm.fields_dict.meal_plan_html.wrapper);
