...
def execute(filters=None):
	...
	for pro in get_data(filters):		
		row = [pro.ax_code, pro.name, pro.project_name,get_gov_dis(pro.project_proposal),
			...
			pro.status, pro.percent_complete, get_indicator(pro.name), get_target_indicator(pro.name)]
		data.append(row)
	...
	if filters.get("project"):
		if not data:
			...
			conditions3 = "  parent = '%s'" % filters.get("project")
			...
			target_ben = frappe.db.sql("""SELECT sum(total) as total FROM `tabIndicator Log`
				WHERE tracking_beneficiaries=1 and  %s group by project """ % conditions, as_dict=True)
			pro = frappe.get_doc("Project",filters.get("project"),fields=["percent_complete", "beneficiaries_individual","customer"])
			pro_donor = frappe.db.sql("""SELECT IFNULL(GROUP_CONCAT(donor SEPARATOR ', '),"") as donor FROM `tabDonors Details`
				WHERE  %s group by parent""" % conditions3, as_dict=True)
			pro_gov = frappe.db.sql("""SELECT IFNULL(GROUP_CONCAT(governorate SEPARATOR ', '),"") as governorate FROM `tabGovernorates`
				WHERE  %s group by parent""" % conditions3, as_dict=True)
			report_summary= [
				{
					"value": pro.estimated_costing,
					"indicator": "Blue",
					"label": _("Budget"),
					"datatype": "Currency",
				},
				...
				},
				{
					...
					"indicator": "Green",
					"label": _("Acheivied BNFs"),
					"datatype": "Float",
				},								
			]
			if pro_donor:
				for cat in pro_donor:
					# frappe.throw(frappe.as_json(cat))
			...
			if pro_gov:
				for cat in pro_gov:
					# frappe.throw(frappe.as_json(cat))
					report_summary.append({
						"value": cat.governorate,
						"indicator": "#92A2A2",
						"label": "Governorates",
						"datatype": "Data",
					})
			if target_ben:
				for cat in target_ben:
					# frappe.throw(frappe.as_json(cat))
					report_summary.append({
						"value": cat.total,
						"indicator": "#92A2A2",
...
def get_indicator(project):	
	programs = frappe.db.sql("""SELECT  SUM(total_achieved) indicator FROM `tabIndicator Log` WHERE tracking_beneficiaries = 1
	 AND project=%(project)s """, {"project": project})
	return programs[0][0]

def get_target_indicator(project):	
	programs = frappe.db.sql("""SELECT  SUM(total) indicator FROM `tabIndicators` WHERE tracking_beneficiaries = 1
	 AND project=%(project)s """, {"project": project})
	return programs[0][0]


@frappe.whitelist()
def cal_beneficiaries(project):
	return {"total_target_bnfs": get_target_indicator(project), "beneficiaries_individual": get_indicator(project)}
