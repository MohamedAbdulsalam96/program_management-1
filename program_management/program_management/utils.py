...
def set_project_name(doc):
	if doc.project and not doc.project_name:
		doc.project_name = frappe.db.get_value("Project", doc.project_name, "project_name")

def update_project(project, details, date=None, cancel=False):
	internal_work_history = {}
	for item in details:
		fieldtype = frappe.get_meta("Employee").get_field(item.fieldname).fieldtype
		new_data = item.new if not cancel else item.current
...
@frappe.whitelist()
def get_project_fields_label():
	fields = []
	for df in frappe.get_meta("Project").get("fields"):
		if df.fieldname in ["expected_start_date", "expected_end_date"]:
				fields.append({"value": df.fieldname, "label": df.label})
	return fields	


@frappe.whitelist()
def get_project_field_property(project, fieldname):
	if project and fieldname:
		field = frappe.get_meta("Project").get_field(fieldname)
		value = frappe.db.get_value("Project", project, fieldname)
...
def validate_active_project(project):
	if frappe.db.get_value("Project", Project, "status") == "Completed":
		frappe.throw(_("Transactions cannot be created for an Inactive Project {0}.").format(
			get_link_to_form("Project", project)), InactiveEmployeeStatusError)