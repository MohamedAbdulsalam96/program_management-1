frappe.ui.form.on(cur_frm.doctype, {
	...
	onload: function(frm){
		if(frm.doc.__islocal){
			if(frm.doctype == "NCE"){
				frm.doc.project_extension = [];
			}
		}
	},
	project: function(frm) {
		// frm.add_fetch("project");
	},
	refresh: function(frm) {
		...
		cur_frm.fields_dict[table].grid.add_custom_button(__('Add Row'), () => {
			if(!frm.doc.project){
				frappe.msgprint(__("Please select Projecr"));
				return;
			}
			frappe.call({
				method: 'program_management.program_management.utils.get_project_fields_label',
				callback: function(r) {
					if(r.message){
...
var show_dialog = function(frm, table, field_labels) {
	...
	d.fields_dict["property"].df.onchange = () => {
		...
		if(!property){return;}
		frappe.call({
			method: 'program_management.program_management.utils.get_project_field_property',
			args: {project: frm.doc.project, fieldname: property},
			callback: function(r) {
				if(r.message){
